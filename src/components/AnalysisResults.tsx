...
export function AnalysisResults({ hospitalName, analysisData, onNavigate, onLogout }: AnalysisResultsProps) {
  const { patient, month, biomarkers, timestamp } = analysisData;
  const results = generateAnalysisResults(biomarkers);
  ...
  const radarData = [
    { subject: 'Cholesterol', A: Math.min(100 - ((parseFloat(biomarkers.cholesterol || '200') - 150) / 100 * 100), 100) },
    { subject: 'Blood Pressure', A: Math.min(100 - ((parseFloat(biomarkers.systolicBp || '120') - 90) / 50 * 100), 100) },
    { subject: 'BMI', A: Math.min(100 - Math.abs(parseFloat(biomarkers.bmi || '25') - 22) / 10 * 100, 100) },
    { subject: 'Lifestyle', A: parseFloat(biomarkers.physicalActivity || '2') * 20 },
    { subject: 'Sleep', A: Math.min(parseFloat(biomarkers.sleepHours || '7') / 8 * 100, 100) },
  ];

  const handleGenerateReport = () => {
    onNavigate('report', { patient, results, biomarkers, timestamp });
  };
