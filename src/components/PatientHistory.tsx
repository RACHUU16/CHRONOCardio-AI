  ...
  const loadPatients = async () => {
    try {
      ...
      if (response.patients && Array.isArray(response.patients)) {
        // Transform patient data to include analysis count and recent analysis data
        const transformedPatients = await Promise.all(
          response.patients.map(async (patient: any) => {
            try {
              ...
              const analysisCount = historyResponse.analyses ? historyResponse.analyses.length : 0;
              const latestAnalysis = historyResponse.analyses && historyResponse.analyses.length > 0 
                ? historyResponse.analyses[0] 
                : null;
              ...
              return {
                ...
                riskLevel: latestAnalysis ? latestAnalysis.riskLevel : (['Low', 'Medium', 'High'][Math.floor(Math.random() * 3)] as 'Low' | 'Medium' | 'High'),
                riskScore: latestAnalysis ? latestAnalysis.riskScore : Math.floor(Math.random() * 100),
                analysisCount,
                registrationDate: patient.registrationDate,
                status: (['Active', 'Follow-up', 'Discharged'][Math.floor(Math.random() * 3)] as 'Active' | 'Follow-up' | 'Discharged')
              };
            } catch (error) {
